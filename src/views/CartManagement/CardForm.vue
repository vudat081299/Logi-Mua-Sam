<template>
  <div class="">
    ...
    <v-col cols="4">
    <div class="card-form card-form__inner">
      <div class="card-input">
        <label for="cardNumber" class="card-input__label">Số thẻ</label>
        <input
          type="tel"
      ...
      <div class="card-input">
        <label for="cardName" class="card-input__label">Tên</label>
        <input
          type="text"
          :id="fields.cardName"
          v-letter-only
      ...
      <div class="card-form__row">
        <div class="card-form__col">
          <div class="card-form__group">
            <label for="cardMonth" class="card-input__label">Ngày hết hạn</label>
            <select
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
      <v-btn color="error" @click="closeAddCard">Trở lại</v-btn>
      <v-btn color="primary" v-on:click="invaildCard">Hoàn thành</v-btn>
      </v-card-actions>
    </div>
    </v-col>
...
<script>
import Card from '@/views/CartManagement/Card'
export default {
  name: 'CardForm',
  directives: {
    'number-only': {
  ...
  methods: {
    ...
    invaildCard () {
      ...
      if (sum % 10 !== 0) {
        alert('Số tài khoản không chính xác')
      } else {
